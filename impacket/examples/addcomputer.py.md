...
This is usually done when the [MachineAccountQuota](https://www.thehacker.recipes/ad/movement/builtins/machineaccountquota) domain-level attribute is set higher than 0 (set to 10 by default), allowing for standard domain users to create and join machine accounts. Alternatively,if the MachineAccountQuota is 0, the utility can still be used if the credentials used match a powerful enough account (e.g. domain administrator).
